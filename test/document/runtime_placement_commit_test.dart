...
void main() {
  group('placement preview → document commit', () {
    ...
    test(
      'NodeCreated + applyUpsert duplicates an unmapped preview quad (regression)',
      () {
        ...
        expect(
          ...
          2,
          reason:
              'document path must add a second runtime node when preview was unmapped',
        );
      },
    );

    test(
      'promoteRuntimeNodeAsEntity keeps a single controller node (placement commit)',
      () {
        final previewQuad = controller.addNode(
          FrameNode(
            center: const ui.Offset(50, 60),
